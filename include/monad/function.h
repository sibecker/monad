...
namespace sib::monad {
    ...
    auto operator|(std::function<R(Args...)> function, Then<Invokable> th)
    {
        using Result = decltype(std::invoke(th, std::move(function) | get()));
        return std::function<Result(Args...)>{
            [function = std::move(function), th = std::move(th)](Args... args) {
                return th(function(std::move(args)...));
            }
        } | flatten();
    }
